...
import { Bookmark, BookmarkCheck, Heart, MessageCircle, Repeat2, Share2, X } from 'lucide-react';
...
export default function BookmarksPage() {
  ...
  const [username, setUsername] = useState<string | null>(null); // ✅ Fix #4: store real username
  ...
  useEffect(() => {
    supabase.auth.getUser().then(async ({ data }) => {
      if (data.user) {
        ...
        // ✅ Fix #4: fetch real username from users table
  ...
  // ✅ Fix #2: toggleLike now also updates the likes count in bookmarks state
  const toggleLike = async (postId: string) => {
    ...
    // Update likes count in local bookmarks state
    setBookmarks((prev) =>
      prev.map((b) =>
        b.posts.id === postId
          ? { ...b, posts: { ...b.posts, likes: b.posts.likes + (isLiked ? -1 : 1) } }
          : b
      )
    );
  };
  ...
  // ✅ Fix #3: clearAllBookmarks only clears UI state if DB operation succeeds
                  ...
                  <div className="flex gap-3">
                    ...
                    <div className="flex-shrink-0">
                      {user?.avatar ? (
                        ...
                        // ✅ Fix #6: use initials instead of hardcoded emoji
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between gap-2">
                        <div className="flex items-center gap-1 min-w-0">
                          {/* ✅ Fix #5: display_name with fallback to username */}
